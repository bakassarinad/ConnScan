...
def extract_destination_address(output):
    ...
    for match in matches:
        ...
        if not not match.replace('.', '').isdigit() or is_public_ip(match):
...
def get_ip_address_from_hostname(hostnames):
    ip_addreses_from_hostnames = []
    try:
        for hostname in hostnames:
             socket.gethostbyname(hostname)
             ip_addreses_from_hostnames.append(hostname)
